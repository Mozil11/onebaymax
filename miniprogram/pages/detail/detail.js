...
Page({
  ...
  onLoad: function (options) {
    var self = this;
    self.getcommentcount(options.id);
  ...
  showLikeImg: function () {
    ...
    wx.cloud.callFunction({
      ...
      if(res.result.data.length!=0){
        ...
        var _likes = res.result.data;
        var likes = []
        for(var i=0;i<_likes.length;i++){
          likes.push(_likes[i].avatarUrl)
        }
        self.setData({
    ...
    if(self.data.hadlike){
      ...
      if (self.data.openid) {
        var data = {
          ...
          postid: parseInt(self.data.postID),
 ...
 playAudio :function() {
  ...
  getcommentcount(postid){
    ...
    wx.cloud.callFunction({
      ...
      console.log(res)
      this.setData({
        total_comments: res.result,
        commentCount: "有" + res.result + "条评论"
    ...
    wx.cloud.callFunction({
      ...
      if(!res.result.hasMore){
        ...
        self.setData({
          ...
          commentsList: [].concat(self.data.commentsList, res.result.data)
        });
      }
    }).catch(e=>{
      console.log(e)
  ...
  formSubmit: function (e) {
    ...
    var commentdate = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+date.getDate();
    if (comment.length === 0) {
      self.setData({
        'dialog.hidden': false,
        'dialog.title': '提示',
      ...
      if (self.data.openid) {
        ...
        arr.unshift([parseInt(postID), self.data.title]);
  ...
  creatArticlePoster: function (appPage, api, util, modalView, poster) {
    ...
    var data = {
      ...
      path: path
        ...
        qrcodeImagePath = 'https://ozil.vip/images/gh_ad55e95d6fed_258.jpg';
